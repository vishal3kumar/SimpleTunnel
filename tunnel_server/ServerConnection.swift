...
class ServerConnection: Connection, StreamDelegate {
	...
	func open(host: String, port: Int) -> Bool {
		simpleTunnelLog("Vishal:: Connection \(identifier) connecting to \(host):\(port)")
        ...
        guard let newReadStream = readStream, let newWriteStream = writeStream else {
            simpleTunnelLog("Vishal:: error in readStream or writeStream")
			return false
		}
        ...
        simpleTunnelLog("Vishal:: open successful ???")
		return true
	}
		...
		switch aStream {
			...
			case readStream!:
				switch eventCode {
					...
					case [.openCompleted]:
						if let serverTunnel = tunnel as? ServerTunnel {
                            simpleTunnelLog("\(identifier): Vishal:: sendOpenResultForConnection")
