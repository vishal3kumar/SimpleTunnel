...
class ServerTunnel: Tunnel, TunnelDelegate, StreamDelegate {
	...
	init(newReadStream: InputStream, newWriteStream: OutputStream) {
        ...
        simpleTunnelLog("Vishal:: new connection!!!!")
	}
	...
	func handleConnectionOpen(properties: [String: Any]) {
        simpleTunnelLog("Vishal:: handleConnectionOpen ")
		guard let connectionIdentifier = properties[TunnelMessageKey.Identifier.rawValue] ,
            ...
            let tunnelLayer = TunnelLayer(rawValue: tunnelLayerNumber as! Int)
                ...
                simpleTunnelLog("Vishal:: handleConnectionOpen Error...")
                return
        }

		switch tunnelLayer {
			case .app:
                simpleTunnelLog("Vishal:: handleConnectionOpen app layer")
				guard let flowKindNumber = properties[TunnelMessageKey.AppProxyFlowType.rawValue] ,
                    let flowKind = AppProxyFlowKind(rawValue: flowKindNumber as! Int)
                        ...
                        simpleTunnelLog("Vishal:: handleConnectionOpen for app layer error: braeking")
                        break }

				switch flowKind {
					case .tcp:
                        simpleTunnelLog("Vishal:: handleConnectionOpen for tcp connection")
						...
						guard let host = properties[TunnelMessageKey.Host.rawValue] as? String,
                            let port = properties[TunnelMessageKey.Port.rawValue] as? Int
                                ...
                                simpleTunnelLog("Vishal:: handleConnectionOpen for tcp connection error breaking")
                                break
                        }
 
                        //let host = "54.67.109.122"
                        //let port = "8080"
                            
                        
                            let newConnection = ServerConnection(connectionIdentifier: connectionIdentifier as! Int, parentTunnel: self)
                        simpleTunnelLog("Vishal:: handleConnectionOpen for tcp connection, openning connection \(host), \(port)")
                        //guard newConnection.open(host: host, port: Int(port)!) else {
                        guard newConnection.open(host: host, port: port) else {

//                                let newConnection = ServerConnection(connectionIdentifier: connectionIdentifier, parentTunnel: self)
//                            guard newConnection.open(host: host, port: Int(port)!) else {
                            simpleTunnelLog("Vishal:: handleConnectionOpen for tcp connection, closing connection")
			...
			case .ip:
                simpleTunnelLog("Vishal:: handleConnectionOpen for ip connection")
				...
				guard newConnection.open() else {
                    simpleTunnelLog("Vishal:: handleConnectionOpen for ip connection, error, closing connection")
                    newConnection.closeConnection(.all)
					break
				}
		}
    ...
    func stream(_ aStream: Stream, handle eventCode: Stream.Event) {
		...
		switch aStream {
			...
			case writeStream!:
                simpleTunnelLog("Vishal:: stream writing")
				switch eventCode {
			...
			case readStream!:
                simpleTunnelLog("Vishal:: stream reading")
                ...
                case [.openCompleted]:
                    simpleTunnelLog("Vishal:: stream open complete. tunnelDidOpen")
    ...
    override func closeTunnel() {
        simpleTunnelLog("Vishal:: Closing Tunnel")
    ...
    override func handleMessage(_ commandType: TunnelCommand, properties: [String: AnyObject], connection: Connection?) -> Bool {
		switch commandType {
			case .open:
                simpleTunnelLog("Vishal:: handleMessage open message received")
                handleConnectionOpen(properties: properties)

			case .fetchConfiguration:
                simpleTunnelLog("Vishal:: handleMessage fetch config received")
    ...
    override func writeDataToTunnel(_ data: Data, startingAtOffset: Int) -> Int {
        simpleTunnelLog("Vishal:: writeDataToTunnel")
...
class ServerDelegate : NSObject, NetServiceDelegate {
    ...
    func netService(_ sender: NetService, didAcceptConnectionWith inputStream: InputStream, outputStream: OutputStream) {
		simpleTunnelLog("Vishal:: Accepted a new connection")
    ...
    func netServiceDidStop(sender: NetService) {
		simpleTunnelLog("Vishal:: Network service stopped")
		exit(0)
	}
}
